export class StateManager {
    ...
    addEvent(dayIndex, event, insertIndex) {
    ...
    removeEvent(dayIndex, eventId) {
        const day = this.days[dayIndex];
        const initialLen = day.events.length;
        day.events = day.events.filter(e => e.id !== eventId);
        if (day.events.length < initialLen)
            this.commitState();
    }
    updateEvent(dayIndex, eventId, updates) {
        const day = this.days[dayIndex];
    ...
    getEvent(dayIndex, eventId) {
        return this.days[dayIndex].events.find(e => e.id === eventId);
    }
    moveEvent(fromDay, toDay, eventId, newIndex) {
